...
def get_embedding_model(model_name: str = EMBEDDING_MODEL) -> SentenceTransformer:
    """Load (and cache in-process) a sentence-transformers model.

    If EMBEDDING_MODEL is a local folder (see download_embedding_model.py),
    this never contacts Hugging Face at all — it's a plain filesystem load.

    If it's still a Hub repo id, this tries the local Hugging Face cache
    first (HF_HUB_OFFLINE=1) so repeat runs never hit the network. Only
    falls back to a real network call the first time a model isn't cached
    yet, or if HF_HUB_OFFLINE is already set by the user (their choice is
